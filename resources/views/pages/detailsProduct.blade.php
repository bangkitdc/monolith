    ...
    <div class="container mx-auto px-6 flex flex-col">
      <div class="md:flex md:items-center">
        <div class="w-full h-64 md:w-1/2 lg:h-96">
          <img class="h-full w-full rounded-md object-cover max-w-lg mx-auto" src="/paper-bag.jpg" allt="product-img">
        </div>
          ...
          <div class="mt-2">
            @if($barang['stok'] > 0)
              ...
              <form method="POST" action="{{ route('addToCart', ['id' => $barang['id']]) }}" class="flex gap-4">
                ...
                <button aria-label="Submit Form" type="submit" class="px-6 bg-blue-500 py-1.5 font-medium text-blue-50 hover:bg-blue-600 text-sm rounded">
                  Add to Cart
                </button>
              </form>
            @else
            ...
            @foreach ($recommendations as $recommendation)
              <div class="w-full max-w-sm mx-auto rounded-md shadow-sm border overflow-hidden" onclick="navigateToProductDetails('{{ $recommendation['id'] }}')">
                <div class="flex items-end justify-end h-56 w-full bg-cover bg-center" style="background-image: url('/paper-bag.jpg')">
                  <button aria-label="Cart" class="p-2 rounded-full bg-blue-600 text-white mx-5 -mb-4 hover:bg-blue-500">
