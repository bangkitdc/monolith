    ...
    <div class="container mx-auto px-6 flex flex-col">
      <div class="md:flex md:items-center">
        <div class="w-full h-64 md:w-1/2 lg:h-96">
          <img class="h-full w-full rounded-md object-cover max-w-lg mx-auto" src="/paper-bag.jpg">
        </div>
        <div class="w-full max-w-lg mx-auto mt-5 md:ml-8 md:mt-0 md:w-1/2">
          <h3 class="text-gray-700 text-lg">{{ $barang['nama'] }} | {{ $barang['kode'] }}</h3>
          ...
          <div class="mt-2">
            @if($barang['stok'] > 0)
              ...
              <form method="POST" action="{{ route('addToCart', ['id' => $barang['id']]) }}" class="flex gap-4">
                @csrf
                <div class="input-counter flex items-center border-gray-100">
                  <button 
                    type="button" 
                    class="minus cursor-pointer rounded-l bg-gray-100 py-2 px-3.5 w-10 duration-100 hover:bg-blue-500 hover:text-blue-50"
                  >
                    <span class=""> - </span>
                  </button>
                  <input 
                  ...
                  <button 
                    type="button" 
                    class="plus cursor-pointer rounded-r bg-gray-100 py-2 px-3.5 w-10 duration-100 hover:bg-blue-500 hover:text-blue-50"
                    data-action="increment"
                  >
                    <span class=""> + </span>
                  </button>
                </div>
                <button type="submit" class="px-6 bg-blue-500 py-1.5 font-medium text-blue-50 hover:bg-blue-600 text-sm rounded">
                  Add to Cart
                </button>
              </form>
            @else
          ...
          <hr class="my-3">
          <h4 class="text-gray-900 font-medium text-sm pb-1">Description</h4>
          ...
          <hr class="my-3">
          <h4 class="text-gray-900 font-medium text-sm pb-1">Contact Us</h4>
          ...
          <div class="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 mt-6">
            @foreach ($recommendations as $recommendation)
              <div class="w-full max-w-sm mx-auto rounded-md shadow-sm border overflow-hidden" onclick="navigateToProductDetails('{{ $recommendation['id'] }}')">
                <div class="flex items-end justify-end h-56 w-full bg-cover bg-center" style="background-image: url('/paper-bag.jpg')">
                  <button class="p-2 rounded-full bg-blue-600 text-white mx-5 -mb-4 hover:bg-blue-500">
                ...
                <div class="px-5 py-3 border-t">
                  <h3 class="text-gray-700 text-sm">{{ $recommendation['nama'] }}</h3>
